...
void osc::startTimer(unsigned long durationInMillis, unsigned long* timer)
{
  ...
  timer[0] = (tp.tv_sec / 1000) + (tp.tv_usec / 1000);
  timer[2] = durationInMillis;
}

uint8_t osc::hasTimedOut(unsigned long* timer)
{
  gettimeofday(&tp, NULL);
  timer[1] = (tp.tv_sec / 1000) + (tp.tv_usec / 1000);
...
uint8_t osc::hasControllTimedOut(char* controllName)
{
	LLNODE* nodePointer = findByName(controllName);
	if(nodePointer!=NULL)
	{
