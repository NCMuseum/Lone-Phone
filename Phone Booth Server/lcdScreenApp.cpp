...
char tmpColour [3] = {0,0,0};

//LEDs
unsigned short int numLeds = 6;
apa102LEDStrip leds = apa102LEDStrip(numLeds, 4, 255);
...
int main(int argc, char **argv)
{
	...
	broadcastOSCAddress.sin_addr.s_addr = inet_addr("192.168.1.255");
...
void *ledOutput(void *args_) 
{
	...
	while(1)
	{
		...
		primaryCO.getColour(cIndex%primaryCO._bandWidth, tmpColour);
		for(pixCounter=0; pixCounter<numLeds; pixCounter++)
		{
			leds.setPixel(pixCounter, tmpColour);
		}
		tx_via_spidev(leds.LEDs, leds._frameLength);
		usleep(100000);
	}
}
